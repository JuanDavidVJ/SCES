...
<div class="container">
	...
	<form action="/recursosReposicion" method="post"  enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<input type="file" name="Evidencias" id="Evidencias" class="form-control" value="{{old('Evidencias')}}">
			@error('Evidencias')
			<small style="color: red;">{{ $message }}</small>
			@enderror
		</div>
		...
		<div class="form-group">
			...
			<select name="SC_ActaComite_FK" id="SC_ActaComite_FK" class="form-control">
				...
				@foreach($actas as $acta)
					<option value="{{$acta->SC_ActaComite_PK_ID}}">{{$acta->citacion->SC_Citacion_NumeroActa}} - {{$acta->SC_ActaComite_Nombre}}</option>
				@endforeach
			</select>
